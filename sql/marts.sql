create view if not exists mart_provider_ops_base as
select
  e.event_id,
  r.reporting_period,
...
create view if not exists mart_validation_summary as
select
  r.reporting_period,
  sf.source_file_name,
  v.issue_type,
...
create view if not exists mart_kpi_snapshot_latest as
select
  r.reporting_period,
  k.kpi_code,
  k.kpi_name,
...
create view if not exists mart_llm_commentary_latest as
select
  d.draft_id,
  d.run_id,
  d.reporting_period,
